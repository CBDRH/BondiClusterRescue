...
abstract: The ongoing COVID-19 Delta variant outbreak in Sydney has resulted in 514 active cases, with 63 individuals in hospital and 18 individuals in ICU, as of 12 July 2021. Current restrictions are in place until Friday 16 July, pending further advise from NSW Health. Here we model observed cases and project our model until 1st September under five potential scenarios for non-pharmaceutical interventions beyond the 12 July end date for current restrictions. Our results suggest that reducing daily incidence to a manageable level before September will require a contined lockdown throughout August during which the net effect of all restrictions is stronger than the level observed in early July. Lifting restrictions on social contacts at the end of the current lockdown period will result in a exponential rise in cases and ultimately prolong the outbreak. Maintaining restrictions at the level observed in early July is unlikely to get the outbreak under control before the start of September.
---
                    ...
                    mutate(
                        ...
                        scenario = factor(rep(rep(1:5, each = xLen), 2), 
                                          ...
                                          labels = c('Lift restrictions', 
                                                     'Relax restrictions but limit transmission probability', 
                                                     'Continue at current level', 
                                                     'Tighten restrictions',
                                                     'Tighten restrictions and limit transmission probability'))
                    )
...
(i) interventions that target the average number of daily contacts between individuals from different age groups, which emulating restrictions on social activity such as stay-at home orders.
...
### Demographics
We assumed a population of 4 million unvaccinated individuals with an age distribution reflecting that of the broader Australian population. Given the relatively low proportion of fully vaccinated individuals to date, and the relatively short time frame in question, the potential role of vaccination to mitigate the current outbreak is negligible. To be conservative, we model a population around ~80 the size of Sydney assuming the remaining ~20% are fully protected. 

### Disease parameters
We set the average duration of the incubation period to 4 days and the average duration of the infectious period to 7 days. Our calibration exercise below explored three values of R~0~: 6.0, 7.0 and 8.0. 
...
1. Lift all restrictions on social contacts, while restrictions targeting transmission probability remain as they are.
...
4. Tighten restrictions on social contacts, while restrictions targeting transmission probability remain as they are.
...
```{r projectionsFig, fig.cap='Observed and projected COVID-19 incidence in NSW (1 June - 1 September 2021)', fig.height=4.0, fig.width=6.5, fig.align='center'}
...
The results suggest that completely lifting restrictions would result in an epidemic rise in cases. Continuing with restrictions at the level they were at in early July will not be sufficient to get the outbreak under control. To reduce daily incidence to a manageable level before September will require a lockdown throughout August where the net effect of all restrictions is stronger than the level observed in early July.


# Sensitivity analysis 

In order to test the robustness of our conclusions to our subjective choice of R~0~ and NPI effectiveness, we repeat the analysis using the alternative specification of R~0~ = 6.0 and NPI effectiveness = 30%. We specify a similar set of five future intervention scenarios starting from the baseline of 30%.
...
```{r sensitivityFig, fig.cap='Sensitivity analysis: Observed and projected COVID-19 incidence in NSW (1 June - 1 September 2021)', fig.height=4.0, fig.width=6.5, fig.align='center'}
